"""Publication-quality figure generators for the LaTeX article."""
# ruff: noqa: I001
...
def _default_dir() -> Path:
    ...
    main_file = cfg("latex", "main_file", "latex/main.tex")
    return Path(main_file).parent / "figures"


def generate_heatmap(figures_dir: Path) -> str:
    ...
    sns.heatmap(data, annot=True, fmt="d", cmap="YlOrRd", xticklabels=labels_x,
                yticklabels=labels_y, ax=ax, linewidths=0.5,
                cbar_kws={"label": "Count / Tokens"})
...
def generate_radar(figures_dir: Path) -> str:
    figures_dir.mkdir(parents=True, exist_ok=True)
    cats = ["Latency", "Token\nEfficiency", "Observability",
            "Extensibility", "BiDi\nSupport", "Ease of Use"]
    n = len(cats)
    angles = [i / n * 2 * np.pi for i in range(n)] + [0]
    scores = {"CrewAI": [8, 8, 7, 9, 8, 9], "LangGraph": [6, 7, 9, 10, 5, 5],
              "AutoGen": [4, 4, 6, 7, 3, 7]}
...
def generate_sankey(figures_dir: Path) -> str:
    figures_dir.mkdir(parents=True, exist_ok=True)
    stages = [
        ("Input\nPrompt", 1_000, "#78909C"), ("Researcher\nAgent", 8_500, "#1565C0"),
        ("Writer\nAgent", 42_000, "#2E7D32"), ("Editor\nAgent", 28_000, "#E65100"),
    ...
    ax.set_title("Token Flow Through the Article-Generation Pipeline",
...
def generate_network(figures_dir: Path) -> str:
    figures_dir.mkdir(parents=True, exist_ok=True)
    ...
    nx.draw_networkx_nodes(g, pos, nodelist=list(agent_cfg),
                           node_size=[agent_cfg[node][0] for node in agent_cfg],
                           node_color=[agent_cfg[node][1] for node in agent_cfg],
                           alpha=0.9, ax=ax)
    nx.draw_networkx_nodes(g, pos, nodelist=list(artifact_cfg), node_size=600,
    ...
    legend = [Line2D([0], [0], marker="o", color="w", markerfacecolor=c,
                     markersize=11, label=name) for name, (_, c) in agent_cfg.items()]
...
def generate_all(figures_dir: Path | None = None) -> list[str]:
    """Generate all 4 pipeline figures; returns list of output paths."""
    d = figures_dir if figures_dir is not None else _default_dir()
    return [fn(d) for fn in [generate_heatmap, generate_radar, generate_sankey, generate_network]]
