...
Final values (placeholders — orchestrator will recalibrate before submission):
  ...
  github     = https://github.com/salah-dev-stu/uoh-sqak-ex03  (confirm with Andalus)
  late       = no

Moodle assignment id=270973 — deadline Friday 12 June 2026, 23:59 Asia/Jerusalem

Run:  uv run python scripts/fill_submission_pdf.py
Outputs:
  uoh-sqak-ex03.docx   (filled cover sheet)
  uoh-sqak-ex03.pdf    (final; what each pair member uploads separately to Moodle)
"""
...
TEMPLATE = REPO / "uoh-rl07-ex03-template.docx"
...
def main() -> int:
    if not TEMPLATE.exists():
        ...
        return 1
    intermediate = REPO / f"{GROUP_CODE}-ex03.docx"
    ...
    pdf = REPO / f"{GROUP_CODE}-ex03.pdf"
